...
<div class="form-group">
                  <label class="control-label">Причина бана::</label>
                     <input name="text" class="form-control" id="bbcode" value="{$smarty.post.text|escape}" type="text" required />
               </div>
...
{capture name=edit_comments}
    ...
    <textarea name="text"{if $smarty.session.device == 'web'} id="summernote" rows="15"{/if} class="form-control">{$row.text|esc|escape}</textarea>
    {if $smile && $smarty.session.device == 'web'}
        <div class="fon menu">    
            <div id="emoticons">
                {foreach from=$smile item=onesmile} 
