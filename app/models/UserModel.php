...
class UserModel extends Base {
    ...
    function index($id) {
        $row = DB::run("SELECT `users`.*, (SELECT COUNT(*) FROM `smiles` WHERE `smiles`.`id`=`users`.`id`) AS `smiles` FROM `users` WHERE `id`='" . $id . "'")->fetch(PDO::FETCH_ASSOC);
        ...
        SmartySingleton::instance()->assign(array(
            ...
            'online' => DB::run("SELECT COUNT(*) FROM `online` WHERE `type`='1' AND `id_user`='" . $row['id'] . "'")->fetchColumn(),
    ...
    function users_admin($id) {
        ...
        SmartySingleton::instance()->display(SMARTY_TEMPLATE_LOAD . '/templates/modules/user/online.tpl');
    }
    ...
    function guest() {
        ...
        SmartySingleton::instance()->display(SMARTY_TEMPLATE_LOAD . '/templates/modules/user/guest.tpl');
    }
